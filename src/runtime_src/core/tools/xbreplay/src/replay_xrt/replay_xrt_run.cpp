...
void replay_xrt::register_run_class_func()
{
  ...
  [this] (replay_xrt& handle, std::shared_ptr<utils::message>msg)
  {
    ...
    auto index = std::stoi(args[0].second, nullptr, utils::base_hex);
  ...
  [this] (replay_xrt& handle, std::shared_ptr<utils::message>msg)
  {
    ...
    int index = std::stoi(args[0].second, nullptr, utils::base_hex);
    int size  = std::stoi(args[1].second);
    auto bytes = static_cast<size_t>(std::stoul(args[2].second, nullptr, utils::base_hex));
  ...
  m_api_map["xrt::run::wait(const milliseconds&)"] =
  [this] (replay_xrt& handle, std::shared_ptr<utils::message>msg)
  {
    const std::vector <std::pair<std::string, std::string>> &args = msg->m_args;
