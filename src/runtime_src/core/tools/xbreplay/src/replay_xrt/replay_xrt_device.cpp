...
void replay_xrt::register_device_class_func()
{
  ...
  m_api_map["xrt::device::device(const string&)"] =
  [this] (replay_xrt& handle, std::shared_ptr<utils::message>msg)
  {
    const std::vector <std::pair<std::string, std::string>> &args = msg->m_args;
  ...
  m_api_map["xrt::device::load_xclbin(const string&)"] =
  [this] (replay_xrt& handle, std::shared_ptr<utils::message>msg)
  {
    const std::vector <std::pair<std::string, std::string>> &args = msg->m_args;
    std::string fp;
    if ((msg->m_is_mem_file_available) && (msg->m_buf.size() > 0))
      fp = save_buf_to_file(msg,".xclbin");
    else
      fp = args[0].second;
