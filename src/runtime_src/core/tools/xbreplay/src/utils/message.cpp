 ...
 */
void message::rm_spaces_frm_str(std::string& str1, std::string& str2)
{
                                          ...
                                          const std::string& str_args_value)
{
  ...
  {
    rm_spaces_frm_str(token1, token2);
...
replay_status message::update_args(const std::pair <std::string&, std::string&> args)
{
  ...
  for (auto &arg : m_args)
  {
    ...
    arg_value.erase(std::remove_if (arg_value.begin(), arg_value.end(), ::isspace), arg_value.end());
 ...
 */
void message::look_for_tag(const std::string& tag)
{
  /* Start Marker */
 ...
 */
void message::get_mem_data(const uint32_t offset)
{
  ...
  if (tag_value == mem_tag_value)
  {
    ...
    /* Resize vector */
    m_buf.resize(mem_size, 0);

    if (mem_size < read_block_size)
    {
      file.read(m_buf.data(), mem_size);
      if (!file)
    ...
    else
    {
      ...
      while (bytes_read < mem_size)
      {
        ...
        file.read(&m_buf[bytes_read], bytes_to_read);
