 ...
 * static objects. However, in this case, the initialization of MapFuncPtr to
 * an empty map is straightforward and does not depend on any other static objects.
 */
// NOLINTNEXTLINE(cert-err58-cpp)
const static std::unordered_map < std::string, void **> MapFuncPtr = {
  ...
  {"xrt::device::~device()", (void **) &dtbl.device.dtor},
...
int router::load_symbols()
{
  ...
  for (unsigned long i = 0; i < num_symbols; ++i)
  {
        ...
        (symbol.st_shndx != SHN_UNDEF))
    {
      ...
      FunMangled[demangled_name] = symbol_name.data();
    }
  }
...
namespace xrt::tools::xbtracer {
  ...
  static void replaceFunction(PIMAGE_THUNK_DATA thunk, void* pFunction)
  {
...
int idt_fixup(void* dummy)
{
  ...
  if (dummy != NULL)
  {
    ...
    imageBase = GetModuleHandleA(filename.c_str());
  }
  else
  ...
  {
    libraryName =
        reinterpret_cast<LPCSTR>(reinterpret_cast<DWORD_PTR>(imageBase) +
    ...
    if (!stricmp(libraryName, "xrt_coreutil.dll"))
    {
      ...
      while (originalFirstThunk->u1.AddressOfData != NULL)
      {
        ...
        auto ptr_itr = MapFuncPtr.find(demangle(functionName->Name));
        if (ptr_itr != MapFuncPtr.end())
        {
          void** temp = ptr_itr->second;
          /* update the original function address in the dispatch table */
          *temp = reinterpret_cast<void*>(firstThunk->u1.Function);
          void* pFunction = GetProcAddress(library, functionName->Name);
          if (pFunction)
          {
            if (inst_debug)
              std::cout << demangle(functionName->Name).c_str() << "\n\tOrg = "
                << std::uppercase << std::hex << std::setw(16)
                << std::setfill('0') << firstThunk->u1.Function << " New = "
                << std::uppercase << std::hex << std::setw(16)
                << std::setfill('0') << (ULONG64)pFunction <<"\n";

            replaceFunction(firstThunk, pFunction);
          }
        }
        else if (inst_debug)
          std::cout << "func :: \"" << demangle(functionName->Name) << "\""
                << "not found in MapFuncPtr\n";
