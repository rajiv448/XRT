...
#define XRT_TOOLS_XBT_LOG_ERROR(str)                        \
  do                                                        \
  {                                                         \
    std::cerr << stringify_args(str, " is NULL @ ",         \
                    __FILE__, ":L", __LINE__, "\n");        \
  }                                                         \
  while (0)
...
#define XRT_TOOLS_XBT_CALL_CTOR(fptr, ...)                  \
  ...
  {                                                         \
    ...
    {                                                       \
      (fptr)(__VA_ARGS__);                                  \
      logger::get_instance().set_pimpl(handle);             \
    }                                                       \
    else                                                    \
...
#define XRT_TOOLS_XBT_CALL_EXT_CTOR(fptr, ...)              \
  ...
  {                                                         \
    ...
    {                                                       \
      (fptr)(__VA_ARGS__);                                  \
      logger::get_instance().set_pimpl(this->get_handle()); \
    }                                                       \
    else                                                    \
      XRT_TOOLS_XBT_LOG_ERROR(#fptr);                       \
  }                                                         \
...
#define XRT_TOOLS_XBT_FUNC_ENTRY(f, ...)                                       \
  ...
  {                                                                            \
    ...
    logger::get_instance().log(trace_type::entry,                              \
        stringify_args(__handle.get(), "|", f) +                               \
...
#define XRT_TOOLS_XBT_FUNC_EXIT(f, ...)                                        \
  ...
  {                                                                            \
    ...
    auto __handle = this->get_handle();                                        \
    logger::get_instance().log(trace_type::exit,                               \
        stringify_args(__handle.get(), "|", f) +                               \
