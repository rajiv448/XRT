...
void bo::sync(xclBOSyncDirection dir, size_t size, size_t offset)
{
  ...
  membuf bo_buf(buffer.data(), (unsigned int)size);
...
bo(const xrt::hw_context& hwctx, size_t sz, access_mode access)
{
  ...
  /* As pimpl will be updated only after ctor call*/
  XRT_TOOLS_XBT_FUNC_ENTRY(dtbl.ext.bo_ctor_cxt_s_a, hwctx.get_handle().get(),
    sz, (int)access);
  XRT_TOOLS_XBT_FUNC_EXIT(dtbl.ext.bo_ctor_cxt_s_a);
}
} // namespace xrt::ext
