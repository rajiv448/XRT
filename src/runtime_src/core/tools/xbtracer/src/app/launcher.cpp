...
std::string get_env(std::string key)
{
  ...
  free(val); // Free the memory allocated by _dupenv_s
  return result;
#else
...
std::string find_library(const std::string& lib_name)
{
  ...
  while (std::getline(path_stream, path, path_delimiter))
  {
    full_path = path + std::string(path_separator) + lib_name;
...
int set_envs(launcher& app)
{
  ...
  app.m_start_time = std::chrono::system_clock::now();
  {
    ...
    oss << ns.count();
    app.m_start_time_env = oss.str();
  }

  if (set_env("START_TIME", app.m_start_time_env.c_str()))
    log_d("Environment variable set successfully: START_TIME = ",
        app.m_start_time_env);
  else
                    ...
                    std::string & extra_lib)
{
  ...
  if (lib_path_child_proc == nullptr)
...
int win_launcher(int& argc, char* argv[])
{
  ...
  */
  app.m_lib_path = find_library(std::string(inst_lib_name));
